...
import com.nexora.hammerscale.net.DnsParser
import com.nexora.hammerscale.net.PacketParser
import com.nexora.hammerscale.net.ParsedPacket
import com.nexora.hammerscale.net.UDPHeader
...
/**
 * SFA copy of UdpHandler — isolated so SF3 edits don't affect SFA.
 */
class SfaUdpHandler(
    ...
    fun handlePacket(packet: ParsedPacket) {
        ...
        val connKey = "${packet.ip.srcAddr.hostAddress}:${udp.srcPort}->${packet.ip.dstAddr.hostAddress}:${udp.dstPort}"
        if (isDns) handleDns(packet, udp, connKey) else handleGenericUdp(packet, udp, connKey)
    }
    ...
    private fun handleDns(packet: ParsedPacket, udp: UDPHeader, connKey: String) {
        ...
        val entry = ConnectionEntry(
            id = connKey, protocol = Protocol.DNS, srcPort = udp.srcPort,
            dstIp = packet.ip.dstAddr.hostAddress ?: "8.8.8.8", dstPort = udp.dstPort,
            dstHost = "DNS: $queryName", status = ConnectionStatus.CONNECTING, dnsQuery = queryName
        )
        ...
        scope.launch {
            try {
                ...
                sock.receive(recvPkt); sock.close()
                ...
                val answers = dnsResp?.answers?.map { rec -> when(rec.type){1->"A: ${rec.data}";28->"AAAA: ${rec.data}";5->"CNAME: ${rec.data}";else->rec.data} } ?: emptyList()
                val updatedEntry = entry.copy(dnsAnswers = answers, status = ConnectionStatus.CLOSED, dstHost = "DNS: $queryName → ${answers.firstOrNull() ?: "NXDOMAIN"}")
                onConnectionEvent(updatedEntry)
                val responsePacket = PacketParser.buildIPv4UDPPacket(dstIp, srcIp, udp.dstPort, udp.srcPort, response)
                writeToVpn(responsePacket)
            } catch (e: Exception) { onStatusChange(connKey, ConnectionStatus.CLOSED) }
        }
    }

    private fun handleGenericUdp(packet: ParsedPacket, udp: UDPHeader, connKey: String) {
        val entry = ConnectionEntry(id = connKey, protocol = Protocol.UDP, srcPort = udp.srcPort, dstIp = packet.ip.dstAddr.hostAddress ?: "?", dstPort = udp.dstPort, status = ConnectionStatus.ACTIVE)
        onConnectionEvent(entry)
        val srcIp = packet.ip.srcAddr.address
        val dstIp = packet.ip.dstAddr.address
        val payload = packet.payload
        ...
        scope.launch {
            try {
                val sock = udpSessions.getOrPut(connKey) { DatagramSocket().also { vpnService.protect(it) } }
                sock.soTimeout = 5000
                val target = InetSocketAddress(packet.ip.dstAddr, udp.dstPort)
                sock.send(DatagramPacket(payload, payload.size, target))
            ...
            } catch (e: Exception) { udpSessions.remove(connKey); onStatusChange(connKey, ConnectionStatus.CLOSED) }
        }
    }

    private fun writeToVpn(data: ByteArray) { try { outStream.write(data) } catch (_: Exception) {} }
    fun shutdown() { scope.cancel(); udpSessions.values.forEach { try{it.close()}catch(_:Exception){} }; udpSessions.clear() }
}
